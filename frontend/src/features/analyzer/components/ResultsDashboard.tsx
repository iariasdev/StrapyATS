...
export const ResultsDashboard: React.FC<ResultsDashboardProps> = ({
  ...
  const { user, signInWithGoogle, getAccessToken } = useAuth();
  ...
  const handleSaveToTracker = async () => {
    ...
    try {
      const companyName = result.company_name || result.rewritten_cv?.experiences?.[0]?.company || 'Empresa Vacante';
      const jobTitle = result.rewritten_cv?.candidate_title || 'Cargo Postulado';
      const token = await getAccessToken();

      const payload = {
        company_name: companyName,
        job_title: jobTitle,
        job_portal: result.job_url?.includes('linkedin') ? 'linkedin' : result.job_url?.includes('getonbrd') ? 'getonboard' : 'manual',
        job_url: result.job_url || null,
        ats_match_score: result.match_score,
        status: 'saved',
        notes: `Auditoría ATS (${result.match_score}% compatibilidad) - ${result.seniority_match}`,
        cv_version: {
          cv_json: result.rewritten_cv,
          interview_questions: result.interview_questions,
          cover_letter: result.cover_letter,
          ats_gaps: result.ats_gaps,
        }
      };
      ...
      if (token && token !== 'demo-token-local') {
          ...
          if (isSupabaseConfigured()) {
            const { data: appData, error: appErr } = await supabase
              ...
              .insert({
                ...
                ats_match_score: payload.ats_match_score,
                status: 'saved',
                notes: payload.notes,
              })
              .select()
        ...
        const existingStr = localStorage.getItem('strapy_ats_local_tracker_apps') || '[]';
        ...
        localStorage.setItem('strapy_ats_local_tracker_apps', JSON.stringify(existing));
      }

      setSavedToTracker(true);
    } catch (err: any) {
      console.error('Error saving to tracker:', err);
      setTrackerError(err?.message || 'No se pudo guardar la postulación.');
    } finally {
  ...
  return (
    <div className="space-y-6 font-sans">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-1 border-b-[2px] border-surface-border no-print">
        ...
        <div className="flex items-center gap-2.5 flex-wrap">
          ...
          {savedToTracker ? (
            <Link
              href="/tracker"
              className="revi-btn h-9 px-4 text-xs bg-emerald-900/60 text-emerald-300 border-[2px] border-emerald-600/50 flex items-center gap-2 shadow-revi-sm font-bold"
            >
              <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" />
              <span>Guardado en Tracker ✓ (Ver Tablero)</span>
            </Link>
          ) : (
            <button
              type="button"
              ...
              className="revi-btn h-9 px-4 text-xs bg-brand-primary hover:bg-brand-hover text-white border-[2px] border-surface-border flex items-center gap-2 shadow-revi-sm transition-all shrink-0 font-bold"
            >
              {isSavingToTracker ? (
                <>
                ...
                <>
                  ...
                  <span>Guardar en mi Tracker</span>
                </>
              )}
            </button>
          )}
      ...
      {trackerError && (
        <div className="p-3 bg-rose-950/30 border border-rose-800 text-xs text-rose-300 rounded font-mono">
          {trackerError}
        </div>
      )}
